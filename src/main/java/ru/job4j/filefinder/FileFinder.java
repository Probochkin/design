...
public class FileFinder {
    public static void main(String[] args)  {
        if (args.length != 4) {
            throw new IllegalArgumentException("Root folder is null. -d=folder path -n=file name, mask, or regular expression " +
    ...
    private static void validate(ArgsName arguments) {
        ...
        if ("mask".equals(arguments.get("t"))) {
            if (!arguments.get("n").startsWith(".")) {
                throw new IllegalArgumentException("The file type is specified incorrectly");
            }
        }
        if ("regex".equals(arguments.get("t"))) {
            if (!arguments.get("n").contains(".")) {
                throw new IllegalArgumentException("The regex is specified incorrectly");
            }
        }
        if ("name".equals(arguments.get("t"))) {
            if (!arguments.get("n").matches(".*.[a-zA-Z]")) {
